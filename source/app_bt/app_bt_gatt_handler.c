...
#include "source/app_hw/task_rtc.h"
#ifdef ENABLE_BT_SPY_LOG
#include "cybt_debug_uart.h"
#endif
// ----------------------------------------------------------
// Alarm Data Structure + Global Storage
// ----------------------------------------------------------


/* function to handle Bluetooth time writes */
static void bt_set_rtc_time(uint8_t hours, uint8_t minutes, uint8_t seconds)
{
    rtc_packet_t rtc_packet;
    
    /* Create the packet */
    rtc_packet.operation = RTC_OP_SET_TIME;
    rtc_packet.return_queue = NULL;  /* No response needed */
    
    /* Convert decimal to BCD and set the time */
    rtc_packet.time_date.hours = decimal_to_bcd(hours);
    rtc_packet.time_date.minutes = decimal_to_bcd(minutes);
    rtc_packet.time_date.seconds = decimal_to_bcd(seconds);
    
    /* Send to the RTC task queue */
        ...
        if (app_gatt_db_ext_attr_tbl[i].handle == attr_handle)
        {
            ...
            if (validLen)
            {
                ...
                *  peer can enable or disable notification or indication */
                ...
                case HDLC_ALARM_ADDALARM_VALUE:
                {
                    printf("-> AddAlarm packet, len=%d\n", len);
                
                    if (len < 2)
                    {
                        printf("ERROR: AddAlarm packet too short! Expected 2 bytes (hour, minute)\n");
                        return WICED_BT_GATT_INVALID_ATTR_LEN;
                    }
                
                    uint8_t hour   = p_val[0];
                    uint8_t minute = p_val[1];
                
                    printf("  Hour   : %d\n", hour);
                    printf("  Minute : %d\n", minute);
                
                     /* Convert decimal to BCD and set the alarm time */
                   ...
                   /* Update the alarm time string with mutex protection */
                    if (alarm_time_mutex != NULL)
                    {
                        ...
                        sprintf(alarm_time_str, "%02u:%02u",hour,minute);
                ...
                case HDLC_ALARM_SETTIME_VALUE:
                {
                printf("-> SetTime packet, len=%d\n", len);
                 if (len < 2)
                    {
                        printf("ERROR: AddAlarm packet too short! Expected 2 bytes (hour, minute)\n");
                        return WICED_BT_GATT_INVALID_ATTR_LEN;
                    ...
                    // Set the RTC time
                    bt_set_rtc_time(hour, minute, 0);
                    break;
                }
            

                case HDLC_ALARM_DELETEALARM_VALUE:
                {
                    printf("-> DeleteAlarm packet, len=%d\n", len);
               
                    if (len != 1)
                    {
                        printf("ERROR: DeleteAlarm bad length! Expected 1 (index)\n");
                        return WICED_BT_GATT_INVALID_ATTR_LEN;
                    }
                   

                    printf("  Delete alarms");
